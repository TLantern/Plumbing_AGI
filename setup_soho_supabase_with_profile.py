#!/usr/bin/env python3
"""
Supabase setup for SoHo Salon Denton using existing profile
"""
...
def setup_soho_salon_supabase():
    """Setup SoHo Salon Denton using existing profile"""
    ...
    try:
        ...
        salon_uuid = "c8b447d7-90ed-42e7-858c-2a667989a52a"
        print(f"🏢 Using existing profile: {salon_uuid}")
        ...
        knowledge_data = {
            ...
            "salon_uuid": salon_uuid,  # Link to existing profile
        ...
        for service in salon_data["services"]:
            service_record = {
                'salon_id': salon_uuid,  # Use existing profile UUID
        ...
        # 5. Update the profile with SoHo Salon information
        ...
        profile_result = supabase.table('profiles').update(profile_update).eq('id', salon_uuid).execute()
        print(f"✅ Profile updated: {len(profile_result.data)} records")
        
        print("\n🎉 SoHo Salon Denton setup complete in Supabase!")
        print(f"📍 Location ID: {salon_data['location_id']}")
        print(f"🏢 Profile UUID: {salon_uuid}")
        ...
        profile_check = supabase.table('profiles').select('salon_name, phone').eq('id', salon_uuid).execute()
        if profile_check.data:
            profile_info = profile_check.data[0]
            print(f"✅ profile updated: {profile_info['salon_name']} - {profile_info['phone']}")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("4. Data is now linked to profile: c8b447d7-90ed-42e7-858c-2a667989a52a")
    else:
