#!/usr/bin/env python3
"""
Direct Supabase setup for SoHo Salon Denton
"""
...
def setup_soho_salon_supabase():
    """Setup SoHo Salon Denton directly in Supabase"""
    ...
    try:
        ...
        services_check = supabase.table('scraped_services').select('id').eq('salon_id', str(salon_data["location_id"])).execute()
        print(f"✅ scraped_services: {len(services_check.data)} records")
        
        # Check salon_info
        info_check = supabase.table('salon_info').select('id').eq('salon_id', str(salon_data["location_id"])).execute()
